...
import sys
import re
sys.path.append('./text_generator')
import string
import pdb
...
def load_dict(lang):
    ...
    lang_dict = []
    with open(os.path.join('./text_generator/dicts', lang + '.txt'), 'r', encoding="utf8", errors='ignore') as d:
...
def load_fonts(lang):
    ...
    if lang == 'cn':
        return [os.path.join('./text_generator/fonts/cn', font) for font in os.listdir('./text_generator/fonts/cn')]
    else:
        return [os.path.join('./text_generator/fonts/latin', font) for font in os.listdir('./text_generator/fonts/latin')]
...
def preProcess(rawText):
    ...
    for i in range(len(rawText)):
        rawText[i] = rawText[i].translate(table)

    # Convert to lowercase
    for i in range(len(rawText)):
        rawText[i] = rawText[i].lower()
        preProcess_text.append(rawText[i])
...
def generate():
    ...
    if args.name_format == 2:
        cleaned_label = preProcess(strings)
        # Create file with filename-to-label connections
        with open(os.path.join(args.output_dir, "true_words.txt"), 'w', encoding="utf8") as f:
            for i in range(string_count):
                file_name = str(i) + "." + args.extension
                f.write("{}\n".format(cleaned_label[i]))
